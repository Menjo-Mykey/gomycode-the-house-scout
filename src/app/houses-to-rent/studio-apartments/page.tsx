...
interface IProperty {
  ...
  mainImage: string;
  gallery: string[];
  agent?: IAgent; // Agent is optional
}
...
export default function StudioApartments() {
  ...
  // Fetch properties data and set states
  useEffect(() => {
    const fetchProperties = async () => {
      try {
        const response = await fetch("/api/properties?type=studio"); // Adjust the query to filter for studio apartments
        if (!response.ok) throw new Error(`Fetch failed with status ${response.status}`);

        const data: IProperty[] = await response.json();

        // Ensure valid data structure and provide defaults
        const updatedData = data.map((property: IProperty) => ({
          ...property,
          mainImage: property.mainImage || `/images/studio1.jpg`, // Use a valid default image
          gallery: property.gallery || [`/images/studio2.jpg`, `/images/studio3.jpg`, `/images/studio4.jpg`],
  ...
  // Function to handle gallery image change
  const changeGalleryImage = (propertyIndex: number, direction: "next" | "prev") => {
    setActiveGalleryIndex((prev) =>
      prev.map((currentIndex, i) =>
        i === propertyIndex
          ? (currentIndex + (direction === "next" ? 1 : -1) + properties[i].gallery.length) % properties[i].gallery.length
          : currentIndex
      )
    );
  };
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8">
      <h2 className="text-3xl sm:text-4xl font-bold mb-6">Studio Apartments</h2>
